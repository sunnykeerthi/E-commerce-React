...
type AutocompleteRef = MutableRefObject<
  Promise<AutocompleteResponse | undefined> | undefined
>;
...
export default function useSearchWithNearMeHandling(
   ...
   */
  const autocompletePromiseRef =
    useRef<Promise<AutocompleteResponse | undefined>>();
  const isVertical =
    answersActions.state.meta.searchType === SearchTypeEnum.Vertical;
  ...
  async function executeQuery() {
    ...
    if (!answersActions.state.location.userLocation) {
      ...
      const autocompleteResponseBeforeSearch =
        await autocompletePromiseRef.current;
    ...
    browserHistory(`/${isVertical ? verticalKey : "all"}?query=${query}`);
  }
