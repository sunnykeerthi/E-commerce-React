...
export function Dropdown(props: PropsWithChildren<DropdownProps>): JSX.Element {
  ...
  const [childrenWithDropdownItemsTransformed, items] =
    getTransformedChildrenAndItemData(children);
  ...
  useLayoutEffect(() => {
    if (
      ...
      parentQuery !== lastTypedOrSubmittedValue
    ) {
...
function useInputContextInstance(initialValue = ""): InputContextType {
  ...
  const [lastTypedOrSubmittedValue, setLastTypedOrSubmittedValue] =
...
function useFocusContextInstance(
  ...
  function updateFocusedItem(updatedFocusedIndex: number, value?: string) {
    ...
    let updatedValue;
    if (
      updatedFocusedIndex === -1 ||
      updatedFocusedIndex >= numItems ||
      numItems === 0
    ) {
...
function getTransformedChildrenAndItemData(
  children: ReactNode
): [ReactNode, DropdownItemData[]] {
  const items: DropdownItemData[] = [];
  const childrenWithDropdownItemsTransformed = recursivelyMapChildren(
    children,
    (child) => {
      ...
      const transformedItem = createElement(DropdownItemWithIndex, {
        ...props,
