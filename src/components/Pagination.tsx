...
const builtInPaginationCssClasses: Readonly<PaginationCssClasses> = {
  ...
  label:
    "z-0 inline-flex items-center px-4 py-2 text-sm font-semibold border border-gray-300 text-neutral",
  selectedLabel:
    "z-10 inline-flex items-center px-4 py-2 text-sm font-semibold border border-primary text-primary bg-primary-light",
  leftIconContainer:
...
export function Pagination(props: PaginationProps): JSX.Element | null {
  ...
  const reportAnalyticsEvent = null;
  ...
  const paginationContainerClassNames = classNames(
    cssClasses.paginationContainer,
    {
      ...
      <nav className={cssClasses.labelContainer} aria-label="Pagination">
        ...
        {paginationLabels.map((label, index) => {
          switch (label) {
            case "...":
              return (
                <div key={index} className={cssClasses.label}>
                  {label}
                </div>
              );
